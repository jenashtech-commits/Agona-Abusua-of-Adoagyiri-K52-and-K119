...
-- Keep numbering continuing on from members you already have.
select setval('member_no_seq', greatest((select count(*) from members), 0));
...
--    service role used by the serverless functions) — only Claude/you
--    running SQL, or the /api functions, can check it.
